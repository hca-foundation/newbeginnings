app.controller("HomeCtrl", ['$scope', '$rootScope', '$http', '$location', '$filter', function ($scope, $rootScope, $http, $location, $filter) {
    ...
    $scope.resendLink = function (item) {
        $scope.surveyLink = {};
        $scope.surveyLink.Link = "https://newbegininingcenter.azurewebsites.net/#!/survey/" + item["Client_SID"] + "/" + item["Token"];
        $scope.surveyLink.Client = item["LastName"] + ", " + item["FirstName"];
    }
    ...
    $scope.copyToClipboard = function (link) {
        ...
        var modal = document.getElementById('modalSurveyLink1');
